...
export default function Header() {
  ...
  const { place, setPlace, measurementSystem, setMeasurementSystem } =
      ...
      <nav className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div className="search w-full md:w-1/2 flex flex-row-reverse items-center mt-2 md:mt-0">
          <div className="search-input w-full">
            <input
              type="text"
              className={`w-full text-center hover:border-b outline-none ${
                dark ? "dark-input" : "light-input"
              }`}
              placeholder="Search..."
              value={text}
          ...
          {search.length > 0 && showSearch && (
            <div
              className={`${
                dark ? "dark-search" : "light-search"
              } absolute z-10 min-w-72 top-0 translate-y-20 sm:translate-y-14 rounded-lg`}
            >
              {search.length > 0 &&
                search.map((result) => (
                  <div
        ...
        <div className="settings flex flex-row items-center absolute md:static end-0 -translate-y-5 md:translate-y-0 -translate-x-4 md:translate-x-0 mt-2 md:mt-0">
          <div
            className="theme cursor-pointer hover:scale-110"
            onClick={themeHandle}
          >
            {dark ? (
          ...
          <div
            className="setting hover:scale-110 ms-4 cursor-pointer"
          ...
          <div
            className={`${showSettings ? "block" : "hidden"} ${
              dark ? "dark-main" : "light-main"
            } p-4 absolute w-72 top-0 end-0 translate-y-16 sm:translate-y-9 rounded-3xl`}
          >
            ...
            <div className={`mt-4 flex flex-row gap-2 flex-wrap items-center`}>
              {Object.values(MEASUREMENT_SYSTEM).map((system) => (
                <div
                  className={`${dark ? "dark-card" : "light-card"} ${
                    system === measurementSystem ? "opacity-100" : "opacity-50"
                  } cursor-pointer w-20 h-8 rounded-lg text-center`}
